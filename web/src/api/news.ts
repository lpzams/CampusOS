...
// ===== 新闻收藏（整合自 CampusOS_a 的「我的收藏」功能） =====

/** 收藏新闻（需登录）：POST /api/news/favorite/{id} */
export function favoriteNews(id: number | string) {
  return post<void>(`/news/favorite/${id}`)
}

/** 取消收藏（需登录）：DELETE /api/news/favorite/{id} */
export function unfavoriteNews(id: number | string) {
  return del<void>(`/news/favorite/${id}`)
}

/** 我的收藏列表（需登录，返回完整新闻对象）：GET /api/news/favorites */
export function getFavorites() {
  return get<NewsItem[]>('/news/favorites')
}
