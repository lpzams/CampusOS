...
public class NewsController {
    ...
    public NewsController(NewsAppService newsAppService, AuthSession session) {
    ...
    @GetMapping("/categories") public Result<List<Map<String,Object>>> categories() { return Result.success(newsAppService.categories()); }
    @Authenticated @PostMapping("/favorite/{id}") public Result<Void> favorite(@PathVariable Long id) { newsAppService.favorite(session.userId(), id); return Result.success(); }
    @Authenticated @DeleteMapping("/favorite/{id}") public Result<Void> unfavorite(@PathVariable Long id) { newsAppService.unfavorite(session.userId(), id); return Result.success(); }
    @Authenticated @GetMapping("/favorites") public Result<List<NewsDTO>> favorites() { return Result.success(newsAppService.favorites(session.userId())); }

    /** 发布新闻（后台管理用） */
    @Authenticated @PostMapping
    public Result<Long> create(@Valid @RequestBody CreateNewsCommand command) {
        return Result.success(newsAppService.create(session.userType(), command));
    }

    /** 下线新闻（后台管理用） */
    @Authenticated @PutMapping("/{id}/offline")
    public Result<Void> offline(@PathVariable Long id) {
        newsAppService.offline(session.userType(), id);
        return Result.success();
    }

    /** 删除新闻（后台管理用） */
    @Authenticated @DeleteMapping("/{id}")
    public Result<Void> delete(@PathVariable Long id) {
        newsAppService.delete(session.userType(), id);
        return Result.success();
    }
}
